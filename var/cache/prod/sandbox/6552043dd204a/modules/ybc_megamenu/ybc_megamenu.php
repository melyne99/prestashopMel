...
//                $errors[] = $this->l('Submenu max-width need to be between 25 and 100');
         ...
         */
         ...
         if($id_block && $this->itemExists('block','id_block',$id_block) && Tools::isSubmit('delimage'))
         {
            ...
            if(file_exists($imageUrl))
            {
                ...
                $block->update();
                $this->_refeshMenuCache();
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&control=block&id_block='.$id_block);
            }
            else
                $errors[] = $this->l('Image does not exist');   
         }
    ...
    public function getMenuData($field)
    {        
        if(Tools::isSubmit('saveMenu'))
    ...
    public function getBlockFields($id_block = 0)
    {
        $params = array();
     ...
     */
     ...
     {       
       ...
       if($oldFiles = glob(dirname(__FILE__).'/images/menu/*'))
        ...
        $this->copyDir(dirname(__FILE__).'/views/img/menu/',dirname(__FILE__).'/images/menu/');
        $this->copyDir(dirname(__FILE__).'/views/img/column/',dirname(__FILE__).'/images/column/');
        $this->copyDir(dirname(__FILE__).'/views/img/block/',dirname(__FILE__).'/images/block/'); 
     }
	...
	{
		$this->context->controller->addCSS($this->_path.'views/css/ybc_megamenu.css','all');
        $this->context->controller->addCSS($this->_path.'views/css/front_custom.css','all');
        if($this->directionClass=='ybc-dir-rtl')
            $this->context->controller->addCSS($this->_path.'views/css/ybc_megamenu_rtl.css','all');   
        $this->context->controller->addJS($this->_path.'views/js/ybc_megamenu.js'); 
