         ...
         if ($('.header_bottom.ybc_float_header').length > 0)
         {
            ...
            var sticky_navigation = function(){
                ...
                if (scroll_top > sticky_navigation_offset_top) {
                    $('.main-menu').addClass('scroll_heading').css({ 'position': 'fixed','background-color': '#fff','z-index':'10'});
                    $('#header').css({'margin-bottom':''+headerFloatingHeight+'px'});
                    $('.main-menu').addClass('has_fixed');
                } else {
                    $('.main-menu').removeClass('scroll_heading').css({ 'position': 'relative','background-color': 'transparent','z-index':'0' });
